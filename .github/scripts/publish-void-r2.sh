...
# This publisher mirrors the one in DankMaterialShell.
set -euo pipefail
...
download() {
    ...
    aws_r2 s3 sync "s3://${R2_BUCKET}/${R2_PREFIX}/current/" "$REPOSITORY_DIR/current/" --only-show-errors
...
build_manifest() {
    ...
    for file in "$REPOSITORY_DIR"/current/*.xbps; do
        ...
        size="$(stat -c '%s' "$file")"
        packages="$(jq --arg name "$name" --arg version "$version" --arg revision "$revision" \
            --arg filename "$filename" --arg sha256 "$sha" --argjson size "$size" \
            '. + [{name: $name, version: $version, revision: $revision, filename: $filename, sha256: $sha256, size: $size}]' \
            <<<"$packages")"
    done
    jq -n --arg repository "$R2_PREFIX" --arg source_commit "$SOURCE_COMMIT" \
        --arg published_at "$PUBLISHED_AT" --argjson packages "$packages" \
...
archive_retired() {
    ...
    for old in "$REPOSITORY_DIR"/previous-current/*.xbps "$REPOSITORY_DIR"/previous-current/*.sig2; do
        ...
        if [[ ! -e "$REPOSITORY_DIR/current/$filename" ]]; then
            aws_r2 s3 cp "$old" "s3://${R2_BUCKET}/${archive_prefix}/${filename}" \
...
upload_current() {
    ...
    for file in "$REPOSITORY_DIR"/current/*.xbps "$REPOSITORY_DIR"/current/*.sig2; do
        filename="$(basename "$file")"
        aws_r2 s3 cp "$file" "s3://${R2_BUCKET}/${R2_PREFIX}/current/${filename}" \
            --cache-control 'public,max-age=31536000,immutable' --only-show-errors
    done
    aws_r2 s3 cp "$REPOSITORY_DIR/current/x86_64-repodata" \
        "s3://${R2_BUCKET}/${R2_PREFIX}/current/x86_64-repodata" --cache-control 'no-cache' --only-show-errors
    aws_r2 s3 cp "$REPOSITORY_DIR/current/manifest.json" \
        "s3://${R2_BUCKET}/${R2_PREFIX}/current/manifest.json" --cache-control 'no-cache' --only-show-errors
    shopt -s nullglob
    for old in "$REPOSITORY_DIR"/previous-current/*; do
        filename="$(basename "$old")"
        if [[ ! -e "$REPOSITORY_DIR/current/$filename" ]]; then
            aws_r2 s3 rm "s3://${R2_BUCKET}/${R2_PREFIX}/current/${filename}" --only-show-errors
        fi
    done
}
...
publish() {
    ...
    (( ${#packages[@]} > 0 )) || { echo "error: refusing to publish an empty XBPS repository" >&2; exit 1; }
    [[ -s "$REPOSITORY_DIR/current/x86_64-repodata" ]] || { echo "error: x86_64-repodata is missing or empty" >&2; exit 1; }
    for file in "${packages[@]}"; do
        [[ -s "${file}.sig2" ]] || { echo "error: signature is missing for $(basename "$file")" >&2; exit 1; }
    done
    verify_immutable_packages
    build_manifest
    archive_retired
